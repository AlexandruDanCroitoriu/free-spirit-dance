...
const source=await readFile('app/components/payment-presets.tsx','utf8');
const data={presets:[{id:1,courseId:null,name:'Four Zouk classes',amountMinor:20050,allocations:[{courseId:1,courseName:'Zouk',allowance:4}]}],courses:[{id:1,name:'Zouk'}]};
try {
 for(const mode of ['list','add','edit']) {
  ...
  const outfile=resolve(directory,mode+'.mjs');await build({stdin:{contents:fixture,resolveDir:resolve('app/components'),loader:'tsx'},outfile,bundle:true,format:'esm',platform:'node',jsx:'automatic',external:['react','react/jsx-runtime']});
  ...
  if(mode!=='list'){assert.match(html,/inset-0[^"]*md:left-auto/);assert.match(html,/Close payment preset panel/);assert.match(html,/<dialog/);assert.match(html,/Preset name/);assert.match(html,/Classes per course/);assert.match(html,/type="checkbox"/);assert.match(html,mode==='edit'?/Edit payment preset/:/Add payment preset/);}
 }
 console.log('PASS: Course payment-preset tab and accessible add/edit popup fields render.');
