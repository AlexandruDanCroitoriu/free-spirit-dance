...
assert.equal(sqlite.prepare("SELECT COUNT(*) AS n FROM course_schedule WHERE course_id = ?").get(created.id).n,3);
response=await collection.POST(request({...input,name:"No start date",startDate:"",endDate:null}));
assert.equal(response.status,201);
assert.equal((await response.json()).startDate,null);
