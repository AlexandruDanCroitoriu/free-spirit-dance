...
let coverage;
const missed = [];
const balance = courseCreditBalance(course, [], occurrences, payment, attendance, new Date('2024-02-01'), (_id, value) => { coverage = value; }, slot => missed.push(slot));
assert.deepEqual(coverage.classes.map(row => row.startsAt), ['2024-01-02T20:00', '2024-01-04T20:00', '2024-01-09T20:00']);
assert.deepEqual(missed, ['2024-01-04T20:00', '2024-01-09T20:00']);
assert.equal(balance.excessAttendance, 1);
...
const sameDay = courseCreditBalance(course, [], occurrences, payment, [], new Date('2024-02-01'));
assert.equal(sameDay.missedClasses, 0, 'A payment does not start a package without attendance');
assert.equal(sameDay.remainingAllowance, 3, 'An unstarted package retains its allowance');
console.log('PASS: Payment coverage alone determines missed classes and credit use.');
