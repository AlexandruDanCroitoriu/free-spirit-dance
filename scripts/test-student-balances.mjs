...
assert.equal(students.length,1,'inactive students and students with no assignment or activity are excluded');
assert.equal(students.some(s=>s.id===1),false,'inactive students are excluded even when they have debt');
assert.equal(students.find(s=>s.id===2).balances[0].remainingAllowance,2,'future payment settles prior attendance');
console.log('PASS: dashboard balances, inactive exclusion, per-course isolation, retroactive payment and refreshed balances.');
