...
assert.equal(coursePreset.name, 'Intermediate Zouk'); assert.equal(coursePreset.amountMinor, 0); assert.equal(coursePreset.allowance, 4);
...
for(const bad of [{...input,name:''},{...input,amount:'invalid'},{...input,allocations:[]},{...input,allocations:[{courseId:1,allowance:0}]},{...input,allocations:[{courseId:1,allowance:1.5}]},{...input,allocations:[{courseId:1,allowance:1},{courseId:1,allowance:2}]}]) assert.equal((await collection.POST(request(bad))).status,400);
