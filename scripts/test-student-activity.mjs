...
assert.equal(helpers.parseAmount('0', true),0);
...
const allLogs = await read();
assert.equal(allLogs.logs.length, 30, 'The ordinary activity request returns the complete log.');
const firstLogs = await read(1, '?logsPage=1');
