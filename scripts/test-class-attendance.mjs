...
let roster=await (await get()).json();assert.equal(roster.students.length,3);assert.deepEqual(roster.students.filter(s=>s.assigned).map(s=>s.id).sort(),[1,3]);assert.equal(roster.students.find(s=>s.id===2).assigned,0);assert.equal(roster.createdBy,null,'A recurring slot has no creator until it is saved');
const historyApi=await import(moduleUrl(readFileSync('app/api/class-attendance/history/route.ts','utf8').replace('import { env } from "../../../lib/storage";', 'const env = globalThis.activityTestEnv;').replace('"../../../lib/class-attendance"', JSON.stringify(classUrl))));
const history=(target=slot)=>historyApi.GET(new Request('https://school.example.test/api/class-attendance/history?'+new URLSearchParams(target)));
assert.deepEqual(await (await history()).json(), []);
...
assert.deepEqual(sqlite.prepare('SELECT action, administrator_email FROM class_change_log ORDER BY id').all().map(row => ({...row})), [{action:'created',administrator_email:'croitoriu.alexandru.code@gmail.com'},{action:'attendance_added',administrator_email:'croitoriu.alexandru.code@gmail.com'},{action:'attendance_added',administrator_email:'croitoriu.alexandru.code@gmail.com'}]);
assert.equal((await (await history()).json()).length,3);
roster=await (await get()).json();assert.equal(roster.students.filter(s=>s.attended).length,2);assert.equal(roster.createdBy,'croitoriu.alexandru.code@gmail.com');
...
assert.deepEqual(sqlite.prepare("SELECT action FROM class_change_log WHERE class_id = (SELECT id FROM classes WHERE course_id=1 AND class_date='2026-09-16' AND start_time='18:30') AND action IN ('cancelled','restored') ORDER BY id").all().map(row=>row.action),['cancelled','restored']);
...
assert.equal(sqlite.prepare('SELECT created_by FROM classes WHERE id = ?').get(createdClass.id).created_by, 'admin@example.test');
assert.equal(sqlite.prepare("SELECT COUNT(*) AS n FROM class_change_log WHERE class_id = ? AND action = 'created'").get(createdClass.id).n, 1, 'Retry does not repeat the creation log');
...
assert.equal(addedRoster.createdBy, 'admin@example.test');
...
assert.deepEqual(sqlite.prepare("SELECT field FROM class_change_log WHERE class_id=? AND action='changed' ORDER BY id").all(createdClass.id).map(row=>row.field), ['end_time','rent_cost','rent_paid']);
...
assert.ok((await (await history(movedSlot)).json()).some(entry=>entry.action==='changed' && entry.field==='date' && entry.administratorEmail==='admin@example.test'));
