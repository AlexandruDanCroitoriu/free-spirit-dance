...
async function request({ development = true, email = owner, cookie = '', method = 'GET', path = '/api/admin-profile', origin, body, catalog = false, host = 'dev-free-spirit-dance.alexandru-croitoriu.dev' } = {}) {
...
test('localhost has main administrator identity and can switch without signing in', async () => {
  for (const host of ['localhost', '127.0.0.1', '[::1]']) {
    ...
    assert.deepEqual(await (await request({ ...options, path: '/api/development-storage' })).json(), { available: true, selected: 'local', catalogAvailable: false });
...
test('catalog routes database and images together and remains owner-only in development', async () => {
  ...
  assert.deepEqual(await (await request({ ...options, host: 'untrusted.example.com' })).json(), { db: 'local-db', images: 'local-images' });
...
test('catalog selection checks readiness before setting a cookie and allows recovery to local', async () => {
  ...
  const unavailable = await request(options);
  assert.equal(unavailable.status, 409);
  assert.equal(unavailable.headers.get('Set-Cookie'), null);
  const ready = await request({ ...options, catalog: true });
  assert.equal(ready.status, 200);
  assert.match(ready.headers.get('Set-Cookie'), /fsd-storage=catalog/);
  assert.equal((await request({ ...options, origin: 'https://untrusted.example.com', catalog: true })).status, 403);
  assert.equal((await request({ cookie: 'fsd-storage=catalog' })).status, 503);
  const recovery = await request({ ...options, cookie: 'fsd-storage=catalog', body: { selected: 'local' } });
  assert.equal(recovery.status, 200);
  assert.match(recovery.headers.get('Set-Cookie'), /fsd-storage=local/);
  assert.deepEqual(await (await request({ catalog: true, path: '/api/development-storage' })).json(), { available: true, selected: 'local', catalogAvailable: true });
});
