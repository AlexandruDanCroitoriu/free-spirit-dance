...
test('localhost has main administrator identity and can switch without signing in', async () => {
  for (const host of ['localhost', '127.0.0.1', '[::1]']) {
    ...
    assert.deepEqual(await (await request({ ...options, path: '/api/development-storage' })).json(), { available: true, selected: 'catalog' });
...
test('catalog selection is the only local development store', async () => {
  ...
  assert.deepEqual(await (await request({ path: '/api/development-storage' })).json(), { available: true, selected: 'catalog' });
});
