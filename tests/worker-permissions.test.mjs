...
const code = stripTypeScriptTypes(readFileSync(new URL('../worker.ts', import.meta.url), 'utf8'))
  ...
  .replace('import { localProductionRequest } from "./app/lib/production-backups/local-production";', 'const localProductionRequest = async (_request, _env, run) => run();').replace('import { productionImages } from "./app/lib/production-backups/production-storage";', 'const productionImages = async (_db, images) => images;').replace('import { withStorage } from "./app/lib/storage";', '')
