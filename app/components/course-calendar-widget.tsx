  ...
  return <>
    ...
    <section data-calendar-range={calendarRange} className={`${calendarRange === "month" ? "mx-auto w-full max-w-6xl" : "w-full"} relative z-20 min-w-0 overflow-visible rounded-2xl border border-stone-200 bg-white shadow-sm`} aria-labelledby="calendar-title">
      ...
      {loading ? <p className="p-8 text-center font-sans text-xs text-slate-400">Loading calendar...</p> : error ? <p className="m-4 rounded-lg border border-red-200 bg-red-50 p-3 font-sans text-xs text-red-700" role="alert">{error}</p> : <div className="min-w-0"><div className="calendar-widget-body relative" ref={calendarBodyRef}>
        ...
        {calendarRange === "year" ? <div className="grid grid-cols-1 gap-3 p-3 sm:grid-cols-2 md:grid-cols-4">{months.map((_, month) => renderYearMonth(visibleMonth.getFullYear(), month))}</div> : calendarRange === "years" ? <div className="grid grid-cols-1 items-start gap-5 p-3 lg:grid-cols-2">{selectableYears.filter((year) => selectedYears.includes(year)).sort((first, second) => first - second).map(renderYearMatrix)}{!multiYears.length && <p className="p-6 text-center font-sans text-sm text-slate-500">{calendarMode === "student" ? "Choose students with activity to show their attendance matrix." : "No calendar activity to show."}</p>}</div> : <><div className="calendar-grid border-b border-stone-200 bg-stone-50">{weekdays.map((day) => <div className="px-1 py-2 text-center font-sans text-[10px] font-bold uppercase tracking-wider text-slate-400" key={day}>{dayLabels[day].slice(0, 3)}</div>)}</div>
