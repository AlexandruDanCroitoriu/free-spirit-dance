...
export default function CourseCalendarWidget() {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    fetch(`/api/student-calendar?studentId=${selectedStudentId}&from=${from}&to=${to}&useRecordedAbsences=${useRecordedAbsences}${calendarRange === "years" ? "&all=true" : ""}`, { signal: controller.signal }).then(async response => {
    ...
    return () => controller.abort();
  }, [calendarMode, selectedStudentId, from, to, calendarRange, calendarReload, useRecordedAbsences]);
  useEffect(() => {
    ...
    Promise.all(selectedStudentIds.map(async (studentId) => {
      const response = await fetch(`/api/student-calendar?studentId=${studentId}&from=${from}&to=${to}&all=true&useRecordedAbsences=${useRecordedAbsences}`, { signal: controller.signal });
      const body = await readJson<{ events: StudentCalendarEvent[] } | ApiError>(response);
      if (!response.ok || !("events" in body)) throw new Error("Could not load student calendar.");
    ...
    return () => controller.abort();
  }, [calendarRange, calendarMode, selectedStudentIds, from, to, calendarReload, useRecordedAbsences]);
  useEffect(() => {
