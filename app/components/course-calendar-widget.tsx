...
export default function CourseCalendarWidget() {
  ...
  const [freeMeetings, setFreeMeetings] = useState<(FreeMeeting & { eventName: string })[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/practice-calendar?from=${from}&to=${to}${calendarRange === "years" ? "&all=true" : ""}`, { signal: controller.signal }).then(async response => {
      const body = await response.json() as { meetings?: (FreeMeeting & { eventName: string })[]; canOpen: boolean; error?: string }; if (!response.ok) throw new Error(body.error ?? 'Could not load events.');
