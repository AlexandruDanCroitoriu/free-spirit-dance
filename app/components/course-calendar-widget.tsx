...
import FreeMeetingCalendarPanel from "./free-meeting-calendar-panel";
...
type StudentCalendarEvent = { date: string; kind: "attendance" | "missed" | "free_missed" | "payment"; courseName: string | null; count: number; paymentId: number | null; complimentary: number; coveredClasses?: { courseName: string; startsAt: string }[] };
...
type CalendarFreeMeeting = FreeMeeting & { eventName: string; eventImagePath: string | null };
...
const matrixStudentColumnWidth = 24;
const sessionEnd = (session: Pick<FreeMeeting, "startsAt" | "durationMinutes">) => new Date(new Date(session.startsAt).getTime() + session.durationMinutes * 60_000).toISOString().slice(0, 16);
// `Catalog FSD.xlsx` → `Prezenta Grupa Mica`. The same workbook sheet spans
// the 2024 and 2025 calendar years, so its full roster belongs to one preset.
const prezentaGrupaMicaStudentIds = [47, 55, 54, 92, 81, 137, 101, 53, 124, 37, 87, 91, 138, 150, 78, 114, 99, 63, 122, 89, 96, 43, 86, 140, 82, 65, 40, 60, 44, 35, 85, 70, 52, 56, 134, 71, 29, 145, 18];
...
const beginners2025StudentIds = [107, 28, 38, 122, 117, 25, 23, 40, 79, 127, 47, 65, 26, 104, 55, 54, 51, 39];
const intermediates2025StudentIds = [40, 44, 46, 47, 49, 52, 53, 56, 79, 81, 92, 96, 101, 108, 125, 130, 137, 143, 144];
const attendancePresetCourses = ["Beginners", "Intermediates", "Improvers"];
const beginners2026Headers = [
  { id: 38, name: "Sorin Burducea" },
  { id: 27, name: "Silvia Bălan" },
  { id: 135, name: "Anca Titerlea" },
  { id: 32, name: "Amin Bensadi" },
  { id: 19, name: "Andrei Amohnoae" },
  { id: 131, name: "Calin Tanase" },
  { id: 109, name: "Alexandra Popovici" },
  { id: 69, name: "Alexandru Georgescu" },
  { id: 95, name: "Victoria Neacsu" },
] as const;
const namedPresets = [
  { name: "incepatori 26", course: "Beginners", year: 2026, studentIds: beginners2026Headers.map((student) => student.id) },
  ...
  { name: "Prezenta Grupa Mica", course: "Beginners", year: 2024, studentIds: prezentaGrupaMicaStudentIds, years: [2024, 2025] },
] as const;
...
function studentEventLabel(event: StudentCalendarEvent) {
  if (event.kind === "free_missed") return `${event.courseName ?? "Class"} · Free missed attendance`;
...
function studentEventColor(event: StudentCalendarEvent, covered = false) {
  if (event.kind === "free_missed") return "border-teal-300 bg-teal-50 text-teal-800";
  return event.kind === "payment" ? "border-cyan-300 bg-cyan-100 text-cyan-900" : event.kind === "attendance" ? event.complimentary ? "border-emerald-400 bg-emerald-100 text-emerald-950 shadow-[0_0_8px_rgba(16,185,129,0.8)]" : covered ? "border-lime-300 bg-lime-100 text-lime-900" : "border-amber-400 bg-amber-100 text-amber-950" : "border-dashed border-orange-500 bg-orange-50 text-orange-950";
}

export default function CourseCalendarWidget() {
  const [selectedDay, setSelectedDay] = useState<string | null>(null);
  const [freeMeetings, setFreeMeetings] = useState<CalendarFreeMeeting[]>([]);
  const [selectedFreeMeeting, setSelectedFreeMeeting] = useState<{ eventId: number; meetingId: number } | null>(null);
  const setSelectedPractice = (_id: number) => undefined;
  ...
  const [attendancePresets, setAttendancePresets] = useState<AttendancePreset[]>([]);
  ...
  const [openedAttendanceCourseName, setOpenedAttendanceCourseName] = useState<string | null>(null);
  ...
  const matrixScrollRefs = useRef(new Map<string, HTMLDivElement>());
  ...
  useEffect(() => {
    ...
    fetch("/api/attendance-presets", { signal: controller.signal }).then(async (response) => {
      const data = await readJson<{ presets: AttendancePreset[] } | ApiError>(response);
      if (!response.ok || !("presets" in data)) throw new Error("Could not load attendance presets.");
      if (!controller.signal.aborted) setAttendancePresets(data.presets);
    }).catch(() => { if (!controller.signal.aborted) setAttendancePresets([]); });
  ...
  useEffect(() => {
    ...
    fetch("/api/students", { signal: controller.signal }).then(async response => {
      ...
      // Historical presets include former students; retain the full directory.
      if (!controller.signal.aborted) setStudents(data.sort((a, b) => `${a.firstName} ${a.lastName}`.localeCompare(`${b.firstName} ${b.lastName}`)));
  ...
  useEffect(() => {
    ...
    fetch(`/api/practice-calendar?from=${from}&to=${to}${calendarRange === "years" ? "&all=true" : ""}`, { signal: controller.signal }).then(async response => {
      const body = await response.json() as { meetings?: CalendarFreeMeeting[]; canOpen: boolean; error?: string }; if (!response.ok) throw new Error(body.error ?? 'Could not load events.');
      if (!controller.signal.aborted) { setFreeMeetings(body.meetings ?? []); setCanOpenEvents(body.canOpen); }
  ...
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [calendarMode, selectedStudentId, from, to, calendarRange, calendarReload]);
  useEffect(() => {
    ...
    return () => controller.abort();
  }, [calendarRange, calendarMode, selectedStudentIds, from, to, calendarReload]);
  useEffect(() => {
  ...
  const coveredCardKeys = new Set(studentEvents.filter((event) => event.kind === "payment").flatMap((event) => (event.coveredClasses ?? []).map((item) => `${item.startsAt.slice(0, 10)}|${item.courseName}`)));
  const highlightedCoverageCardKeys = new Set(visiblePaymentEvents.flatMap((event) => (event.coveredClasses ?? []).map((item) => `${item.startsAt.slice(0, 10)}|${item.courseName}`)));
  ...
  function applyPreset(preset: { course: string; year: number; studentIds: readonly number[]; years?: readonly number[] }) {
    ...
    for (const element of matrixScrollRefs.current.values()) element.scrollLeft = 0;
    setMatrixScrollOffsets({});
  ...
  const isBeginners2026Roster = selectedStudentIds.length === beginners2026Headers.length && selectedStudentIds.every((id, index) => id === beginners2026Headers[index].id);
  const selectedMatrixStudents = selectedStudentIds.map((id) => students.find((student) => student.id === id)).filter((student): student is Student => Boolean(student)).map((student) => ({ ...student, matrixName: (isBeginners2026Roster ? beginners2026Headers.find((header) => header.id === student.id)?.name : undefined) ?? `${student.firstName} ${student.lastName}` }));
  ...
  function renderYearMonth(year: number, month: number) {
    ...
    return <section key={month} className="overflow-hidden rounded-lg border border-stone-300 bg-white">
      ...
      <div className="calendar-grid">{Array.from({ length: leadingDays }, (_, index) => <div className="min-h-10 border-b border-r border-stone-100 bg-stone-50/60" key={`blank-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => {
        ...
        const dayMeetings = calendarMode === "school" ? freeMeetings.filter(m => m.startsAt.slice(0, 10) === dateKey) : [];
        const dayPractices: any[] = [];
        const cancelledSlots = calendarMode === "student" ? cancelledSlotsOn(dateKey) : [];
        const current = sameDate(date, today);
        return <div className={`relative min-h-10 border-b border-r border-stone-100 p-0.5 ${current ? "bg-lime-50" : "bg-white"}`} key={dateKey}><span className={`block text-[8px] font-semibold ${current ? "text-lime-700" : "text-slate-500"}`}>{date.getDate()}</span><div className="mt-0.5 space-y-0.5">{calendarMode === "student" && selectedStudentId !== null && dayStudentEvents.map((event, eventIndex) => { const cardKey = `${event.date}|${event.courseName}`; const covered = event.kind !== "payment" && coveredCardKeys.has(cardKey); const coverageHighlighted = event.kind !== "payment" && highlightedCoverageCardKeys.has(cardKey); const common = `relative z-20 block w-full rounded border px-0.5 py-px text-left font-sans text-[7px] font-bold leading-tight ${coverageHighlighted ? "ring-1 ring-cyan-500" : ""} ${studentEventColor(event, covered)}`; return event.kind === "attendance" || event.kind === "missed" || event.kind === "free_missed" ? <button type="button" key={`${event.kind}-${event.courseName}-${eventIndex}`} ref={(element) => { if (element) activityCardRefs.current.set(cardKey, element); else activityCardRefs.current.delete(cardKey); }} onClick={() => { setOpenedAttendanceCourseName(event.courseName); setOpenedAttendanceDate(dateKey); }} className={common} title={`Open ${studentEventLabel(event)} in the student log`}>{event.kind === "attendance" ? "A" : event.kind === "free_missed" ? "F" : "M"}</button> : <div key={`${event.kind}-${event.courseName}-${eventIndex}`} ref={(element) => { if (element && event.paymentId !== null) paymentCardRefs.current.set(cardKey, element); else if (event.paymentId !== null) paymentCardRefs.current.delete(cardKey); }} className={common} title={studentEventLabel(event)} role="button" onClick={event.paymentId !== null ? () => setOpenedPaymentId(event.paymentId) : undefined} onKeyDown={event.paymentId !== null ? (key) => { if (key.key === "Enter" || key.key === " ") { key.preventDefault(); setOpenedPaymentId(event.paymentId!); } } : undefined} onBlur={() => setHoveredPayment(null)} onFocus={(focus) => setHoveredPayment({ event, element: focus.currentTarget })} onMouseEnter={(hover) => setHoveredPayment({ event, element: hover.currentTarget })} onMouseLeave={() => setHoveredPayment(null)} tabIndex={0}>P</div>; })}{calendarMode === "student" && cancelledSlots.map((slot) => <span key={`${slot.courseId}-${slot.startTime}`} className="block w-full truncate rounded border border-stone-300 bg-stone-100 px-0.5 py-px text-left font-sans text-[7px] font-bold leading-tight text-stone-500" title={`${slot.courseName} · Cancelled · ${displayTime(slot.startTime)}–${displayTime(slot.endTime)}`}>C</span>)}{dayMeetings.map(m => <button type="button" key={`free-meeting-${m.id}`} disabled={!canOpenEvents} onClick={() => setSelectedFreeMeeting({ eventId: m.eventId, meetingId: m.id })} className={`relative z-20 flex w-full items-center gap-0.5 overflow-hidden rounded border border-sky-200 bg-sky-50 px-0.5 py-px text-left font-sans text-[7px] font-bold leading-tight text-sky-900 ${m.cancelled ? 'opacity-50' : ''}`} title={`${m.eventName} · ${m.name} · ${m.startsAt.slice(11,16)}–${sessionEnd(m).slice(11,16)}`}>{m.eventImagePath && <img className="size-3 shrink-0 rounded-sm object-cover" alt="" src={m.eventImagePath} />}<span className="min-w-0 flex-1 truncate">{m.name}{m.cancelled ? ' · Cancelled' : ''}</span></button>)}{calendarMode === "school" && dayPractices.map((session) => <button type="button" key={`practice-${session.id}`} disabled={!canOpenEvents} onClick={() => setSelectedPractice(session.id)} className={`block w-full truncate rounded border border-pink-200 bg-pink-50 px-0.5 py-px text-left font-sans text-[7px] font-bold leading-tight text-pink-800 ${session.cancelled ? "opacity-50" : ""}`} title={`Practice party${session.cancelled ? " · Cancelled" : ""} · ${session.startsAt.slice(11)}–${sessionEnd(session).slice(11)}`}>Practice{session.cancelled ? " · Cancelled" : ""}</button>)}{calendarMode === "school" && daySlots.map((slot) => { const cancelled = courses.find((course) => course.id === slot.courseId)?.cancellations?.some((item) => item.classDate === dateKey && item.startTime === slot.startTime); return <button type="button" key={`${slot.courseId}-${slot.startTime}`} className={`block w-full truncate rounded border px-0.5 py-px text-left font-sans text-[7px] font-bold leading-tight ${cancelled ? "border-stone-300 bg-stone-100 text-stone-500" : "border-lime-200 bg-lime-50 text-lime-800"}`} title={`${slot.courseName}${cancelled ? " · Cancelled" : ""} · ${displayTime(slot.startTime)}–${displayTime(slot.endTime)}`} onClick={() => { const course = courses.find((course) => course.id === slot.courseId); if (course) setSelectedClass({ course, date, slot }); }}>{slot.courseName}{cancelled ? " · Cancelled" : ""}</button>; })}</div></div>;
      })}</div>
    </section>;
  }
  ...
  function renderAttendanceMatrixHeader(year: number, course: Course, scrollLeft: number) {
    ...
    return <div className="sticky top-0 z-30 overflow-hidden bg-stone-50 shadow-sm">
      ...
      <div className="relative h-32 overflow-hidden">
        <div className="absolute top-0 flex h-32 text-left font-sans text-[10px] text-slate-600" style={{ width: `${selectedMatrixStudents.length * matrixStudentColumnWidth}px`, transform: `translateX(${98 - Math.round(scrollLeft)}px)` }}>{selectedMatrixStudents.map((student) => { const selected = selectedMatrixStudentId === student.id; return <div key={student.id} className={`h-32 w-6 shrink-0 border-b border-r border-stone-200 text-center font-bold antialiased ${selected ? "border-x-2 border-sky-600 bg-sky-50 text-sky-950" : "bg-stone-50"}`} title={student.matrixName}><button type="button" aria-pressed={selected} onClick={() => setSelectedMatrixStudentId((id) => id === student.id ? null : student.id)} className="relative flex h-32 w-6 items-start justify-center pt-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-600"><span className="absolute bottom-7 inline-block whitespace-nowrap" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>{student.matrixName}</span>{student.picture ? <img src={student.picture} alt="" className="absolute bottom-1 size-5 rounded-full border border-white object-cover shadow" /> : <span aria-hidden="true" className="absolute bottom-1 flex size-5 items-center justify-center rounded-full bg-slate-200 text-[7px] font-bold text-slate-600">{student.firstName[0]}{student.lastName[0]}</span>}</button></div>; })}</div>
  ...
  function renderMatrixCourse(year: number, course: Course) {
    ...
    return <section key={course.id} className="min-w-0 self-start bg-white"><div ref={(element) => { if (element) matrixScrollRefs.current.set(key, element); else matrixScrollRefs.current.delete(key); }} className="overflow-x-auto" onScroll={(event) => { const scrollLeft = Math.round(event.currentTarget.scrollLeft); setMatrixScrollOffsets((offsets) => ({ ...offsets, [key]: scrollLeft })); }}>{displayedMonths.map((month) => renderAttendanceMatrix(year, month, course, false))}</div></section>;
  }
  function renderYearMatrix(year: number) {
    const yearCourses = coursesForYear(year);
    const columns = yearCourses.length === 1 ? "grid-cols-1" : "grid-cols-1 lg:grid-cols-2";
    return <section key={year} className="min-w-0 overflow-visible rounded-xl border border-stone-200 bg-stone-50/50">
  ...
  return <>
    ...
    {selectedFreeMeeting && <FreeMeetingCalendarPanel {...selectedFreeMeeting} onClose={() => { setSelectedFreeMeeting(null); setCalendarReload(v => v + 1); }} />}
    ...
    {selectedStudentId !== null && (openedAttendanceDate || openedPaymentId !== null) && <StudentPanel key={`calendar-activity-${selectedStudentId}-${openedAttendanceDate ?? "payment"}-${openedAttendanceCourseName ?? ""}-${openedPaymentId ?? ""}`} id={selectedStudentId} attendanceDate={openedAttendanceDate ?? undefined} attendanceCourseName={openedAttendanceCourseName} targetPaymentId={openedPaymentId ?? undefined} onClose={() => { setOpenedAttendanceDate(null); setOpenedAttendanceCourseName(null); setOpenedPaymentId(null); }} onUpdate={() => {}} onDelete={() => { setOpenedAttendanceDate(null); setOpenedAttendanceCourseName(null); setOpenedPaymentId(null); }} />}
    <section data-calendar-range={calendarRange} className={`${calendarRange === "month" ? "mx-auto w-full max-w-6xl" : "w-full"} relative z-20 min-w-0 overflow-visible rounded-2xl border border-stone-200 bg-white shadow-sm`} aria-labelledby="calendar-title">
      <div className="calendar-toolbar relative z-50 flex flex-wrap items-center justify-between gap-2 border-b border-stone-200 px-2 py-2 md:gap-3 md:px-4 md:py-3">
        <div className="flex flex-wrap items-center gap-2 font-sans text-xs"><h2 className="sr-only" id="calendar-title">Calendar</h2>
          ...
          {calendarRange === "years" && calendarMode === "student" && <div ref={presetMenuRef}><button type="button" aria-haspopup="menu" aria-expanded={presetMenuOpen} className="flex h-8 items-center gap-2 rounded-md border border-stone-300 bg-white px-2 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-lime-600" onClick={() => setPresetMenuOpen((open) => !open)}>Student presets <span aria-hidden="true">⌄</span></button>{presetMenuOpen && <div role="menu" aria-label="Student presets" className="absolute inset-x-2 top-full z-40 mt-1 grid max-h-[min(32rem,60dvh)] grid-cols-1 gap-px overflow-y-auto overscroll-contain rounded-md border border-stone-300 bg-stone-200 shadow-lg md:inset-x-4 md:grid-cols-[repeat(auto-fit,minmax(14rem,1fr))]">{attendancePresetCourses.map((course) => <section key={course} className="min-w-0 bg-white p-2 [overflow-wrap:anywhere]"><p className="mb-1 px-2 text-[10px] font-bold uppercase tracking-wide text-slate-500">{course}</p>{attendancePresets.filter((preset) => preset.course === course).map((preset) => <button role="menuitem" type="button" key={`${preset.course}-${preset.year}`} className="block w-full rounded px-2 py-1.5 text-left text-xs font-semibold text-slate-700 hover:bg-lime-50 focus:bg-lime-50 focus:outline-none" onClick={() => applyPreset(preset)}>{preset.course} · {preset.year}<span className="ml-1 font-normal text-slate-400">· {preset.studentIds.length}</span></button>)}</section>)}<section className="min-w-0 bg-white p-2 [overflow-wrap:anywhere]"><p className="mb-1 px-2 text-[10px] font-bold uppercase tracking-wide text-slate-500">armin.xlsx</p>{namedPresets.map((preset) => <button role="menuitem" type="button" key={preset.name} className="block w-full rounded px-2 py-1.5 text-left text-xs font-semibold text-slate-700 hover:bg-lime-50 focus:bg-lime-50 focus:outline-none" onClick={() => applyPreset(preset)}>{preset.name}<span className="ml-1 font-normal text-slate-400">· {preset.studentIds.length}</span></button>)}</section></div>}</div>}
        ...
        <div className="flex items-center gap-1.5 font-sans">
          ...
          {calendarRange === "years" && <span className="hidden px-1 text-xs font-semibold text-slate-600 md:inline">All relevant years</span>}
          ...
          <details className="relative"><summary aria-label="Calendar color legend" title="Calendar color legend" className="flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-md border border-stone-300 bg-white font-semibold text-slate-700 hover:bg-stone-50 focus:outline-none focus:ring-2 focus:ring-lime-600"><span aria-hidden="true" className="flex h-4 w-4 items-center justify-center rounded-full border border-slate-500 text-[10px] font-bold">i</span></summary><div className="absolute right-0 z-40 mt-1 w-60 rounded-md border border-stone-300 bg-white p-3 text-slate-700 shadow-lg"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">Attendance colors</p><ul className="space-y-2"><li className="flex items-center gap-2"><span aria-hidden="true" className="h-4 w-6 rounded border border-lime-300 bg-lime-100 ring-2 ring-cyan-500 ring-offset-1" /><span>Attendance covered by a payment</span></li><li className="flex items-center gap-2"><span aria-hidden="true" className="h-4 w-6 rounded border border-amber-400 bg-amber-100" /><span>Attendance not covered by a payment</span></li><li className="flex items-center gap-2"><span aria-hidden="true" className="h-4 w-6 rounded border border-dashed border-orange-500 bg-orange-50" /><span>Missed class using attendance</span></li><li className="flex items-center gap-2"><span aria-hidden="true" className="h-4 w-6 rounded border border-teal-300 bg-teal-50" /><span>Free missed class</span></li><li className="flex items-center gap-2"><span aria-hidden="true" className="h-4 w-6 rounded border border-cyan-300 bg-cyan-100" /><span>Payment</span></li></ul><p className="mt-3 text-[10px] leading-4 text-slate-500">The cyan outline and lines identify the classes linked to the selected payment.</p></div></details>
        </div>
      </div>
      ...
      {loading ? <p className="p-8 text-center font-sans text-xs text-slate-400">Loading calendar...</p> : error ? <p className="m-4 rounded-lg border border-red-200 bg-red-50 p-3 font-sans text-xs text-red-700" role="alert">{error}</p> : <div className="min-w-0"><div className="calendar-widget-body relative" ref={calendarBodyRef}>
        ...
        {calendarRange === "year" ? <div className="grid grid-cols-1 gap-3 p-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">{months.map((_, month) => renderYearMonth(visibleMonth.getFullYear(), month))}</div> : calendarRange === "years" ? <div className={`grid grid-cols-1 items-start gap-5 p-3 ${selectableYears.filter((year) => selectedYears.includes(year)).length > 1 ? "lg:grid-cols-2" : ""}`}>{selectableYears.filter((year) => selectedYears.includes(year)).sort((first, second) => first - second).map(renderYearMatrix)}{!multiYears.length && <p className="p-6 text-center font-sans text-sm text-slate-500">{calendarMode === "student" ? "Choose students with activity to show their attendance matrix." : "No calendar activity to show."}</p>}</div> : <><div className="calendar-grid border-b border-stone-200 bg-stone-50">{weekdays.map((day) => <div className="px-1 py-2 text-center font-sans text-[10px] font-bold uppercase tracking-wider text-slate-400" key={day}>{dayLabels[day].slice(0, 3)}</div>)}</div>
        ...
        const current = sameDate(date, today); const inMonth = date.getMonth() === visibleMonth.getMonth(); return <section className={`calendar-widget-day relative border-b border-r border-stone-200 p-0.5 sm:p-1.5 ${current ? "bg-lime-50/70" : inMonth ? "bg-white" : "bg-stone-50/70"}`} key={date.toISOString()} aria-label={date.toLocaleDateString()}>
          {calendarMode === "school" && <button type="button" aria-label={`Add event on ${dateKey}`} onClick={() => setSelectedDay(dateKey)} className="absolute inset-0 z-10 rounded-sm hover:bg-lime-100/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-lime-600" />}
          ...
          <div className="space-y-1">{calendarMode === "student" && selectedStudentId !== null && dayStudentEvents.map((event, index) => { const cardKey = `${event.date}|${event.courseName}`; const covered = event.kind !== "payment" && coveredCardKeys.has(cardKey); const coverageHighlighted = event.kind !== "payment" && highlightedCoverageCardKeys.has(cardKey); const cardClass = `relative z-20 ${coverageHighlighted ? "ring-2 ring-cyan-500 ring-offset-1" : ""}`; return event.kind === "attendance" ? <button type="button" key={`${event.kind}-${event.courseName}-${index}`} ref={(element) => { if (element) activityCardRefs.current.set(cardKey, element); else activityCardRefs.current.delete(cardKey); }} onClick={() => setOpenedAttendanceDate(dateKey)} className={`${cardClass} block w-full rounded border p-1 text-left font-sans text-[9px] font-bold transition-colors hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-lime-600 ${studentEventColor(event, covered)}`} title={`Open ${event.courseName ?? "class"} attendance in the student log`}>{studentEventLabel(event)}</button> : <div key={`${event.kind}-${event.courseName}-${index}`} ref={event.kind === "missed" ? (element) => { if (element) activityCardRefs.current.set(cardKey, element); else activityCardRefs.current.delete(cardKey); } : event.kind === "payment" ? (element) => { if (element && event.paymentId !== null) paymentCardRefs.current.set(event.paymentId, element); else if (event.paymentId !== null) paymentCardRefs.current.delete(event.paymentId); } : undefined} className={`${cardClass} rounded border p-1 font-sans text-[9px] font-bold ${studentEventColor(event, covered)}`} role={event.kind === "payment" ? "button" : undefined} onClick={event.kind === "payment" && event.paymentId !== null ? () => setOpenedPaymentId(event.paymentId) : undefined} onKeyDown={event.kind === "payment" && event.paymentId !== null ? (key) => { if (key.key === "Enter" || key.key === " ") { key.preventDefault(); setOpenedPaymentId(event.paymentId!); } } : undefined} onBlur={() => setHoveredPayment(null)} onFocus={event.kind === "payment" ? (focus) => setHoveredPayment({ event, element: focus.currentTarget }) : undefined} onMouseEnter={event.kind === "payment" ? (hover) => setHoveredPayment({ event, element: hover.currentTarget }) : undefined} onMouseLeave={event.kind === "payment" ? () => setHoveredPayment(null) : undefined} tabIndex={event.kind === "payment" ? 0 : undefined}><span className="block">{studentEventLabel(event)}</span></div>; })}{dayMeetings.map(m => <button type="button" key={`free-meeting-${m.id}`} disabled={!canOpenEvents} onClick={() => setSelectedFreeMeeting({ eventId: m.eventId, meetingId: m.id })} className={`relative z-20 flex w-full items-stretch gap-1 overflow-hidden rounded border border-sky-200 bg-sky-50 p-1 text-left font-sans text-[9px] text-sky-900 ${m.cancelled ? 'opacity-50' : ''}`} title={`${m.eventName} · ${m.name} · ${m.startsAt.slice(11,16)}–${sessionEnd(m).slice(11,16)}`}>{m.eventImagePath && <img className="w-5 shrink-0 rounded-sm object-cover" alt="" src={m.eventImagePath} />}<span className="min-w-0 flex-1"><span className="block truncate font-bold">{m.name}{m.cancelled ? ' · Cancelled' : ''}</span><span className="block truncate">{m.eventName} · {m.startsAt.slice(11,16)}</span></span></button>)}{dayPractices.map(s => <button type="button" key={`practice-${s.id}`} disabled={!canOpenEvents} onClick={() => setSelectedPractice(s.id)} className={`relative z-20 block w-full rounded border border-pink-200 bg-pink-50 p-1 text-left font-sans text-[9px] ${s.cancelled ? 'opacity-50' : ''}`} title={`Practice party · ${s.startsAt.slice(11)}–${sessionEnd(s).slice(11)}`}><span className="block truncate font-bold">Practice party{s.cancelled ? ' · Cancelled' : ''}</span><span>{s.startsAt.slice(11)}</span></button>)}{daySlots.map((slot, index) => { const cancelled = courses.find((course) => course.id === slot.courseId)?.cancellations?.some((item) => item.classDate === dateKey && item.startTime === slot.startTime); return <button className={`relative z-20 block w-full min-w-0 rounded border px-0.5 py-1 sm:px-1.5 text-left focus:outline-none focus:ring-2 ${cancelled ? "border-stone-300 bg-stone-100 hover:border-stone-400 focus:ring-stone-500" : "hover:border-lime-400 focus:ring-lime-600"} ${cancelled ? (inMonth ? "" : "opacity-60") : inMonth ? "border-lime-200 bg-lime-50" : "border-stone-200 bg-white/60 opacity-60"}`} key={`${date.toISOString()}-${slot.courseId}-${slot.startTime}-${index}`} onClick={() => { const course = courses.find(({ id }) => id === slot.courseId); if (course) setSelectedClass({ course, date, slot }); }} title={`${slot.courseName} · ${displayTime(slot.startTime)}–${displayTime(slot.endTime)}`} type="button"><span className="block truncate font-sans text-[9px] font-bold leading-3 text-slate-800">{slot.courseName}{cancelled ? " · Cancelled" : ""}</span><span className={`block truncate font-sans text-[8px] font-semibold leading-3 ${cancelled ? "text-stone-500" : "text-lime-700"}`}>{displayTime(slot.startTime)}–{displayTime(slot.endTime)}</span></button>; })}</div>
        </section>; })}</div></>}
      </div></div>}
    </section>
  </>;
