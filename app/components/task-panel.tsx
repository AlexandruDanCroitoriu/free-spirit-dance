...
export default function TaskPanel({ task, board, studentId, listId, onClose, onSaved }: {
  ...
  const studentChoices = students.map(student => ({ id: student.id, name: `${student.firstName} ${student.lastName}${student.nickname.trim() ? ` (${student.nickname.trim()})` : ''}`, picture: student.picture }));
    ...
    <form className="space-y-4 font-sans text-sm" onSubmit={event => { event.preventDefault(); if (!deleting) void submit(); }}>
      ...
      <fieldset disabled={saving || uncertain || missing} className="m-0 space-y-4 border-0 p-0">
        ...
        <section className="py-4" aria-labelledby="task-description-label">
          ...
          {descriptionOpen ? <><TaskDescriptionEditor administrators={administratorChoices} courses={courseChoices} students={studentChoices} events={eventChoices} meetings={meetingChoices} value={descriptionDraft} onChange={setDescriptionDraft} disabled={saving || uncertain || missing} /><div className="mt-2 flex gap-2"><button type="button" disabled={descriptionDraft.length > 50000} className={`${button} bg-blue-400 text-slate-950`} onClick={() => { setFields({ ...fields, description: descriptionDraft }); setDescriptionOpen(false); }}>Save description</button><button type="button" className={button} onClick={() => setDescriptionOpen(false)}>Cancel</button></div><p className="text-xs text-stone-400">Changes are saved with Save task.</p></> : <div role="button" tabIndex={saving || uncertain || missing ? -1 : 0} aria-label="Edit description" onClick={() => { if (saving || uncertain || missing) return; setDescriptionDraft(fields.description); setDescriptionOpen(true); }} onKeyDown={event => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); event.currentTarget.click(); } }} className="min-h-20 w-full cursor-text break-words rounded-md border border-white/30 px-3 py-3 text-left text-stone-300 hover:bg-white/5">{fields.description ? <TaskDescriptionEditor administrators={administratorChoices} students={studentChoices} readOnly value={fields.description} /> : 'Add a more detailed description…'}</div>}
        </section>
      </fieldset>
