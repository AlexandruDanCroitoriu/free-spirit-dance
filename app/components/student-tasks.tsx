...
export default function StudentTasks({ studentId }: { studentId: number }) {
  ...
  async function unlink(key: string) {
    ...
    try {
      const result = await taskRequest<{ revision: number }>(`/api/tasks/${encodeURIComponent(key)}`, 'PATCH', { studentId: null, revision: board.revision });
  ...
  return <section aria-label="Linked tasks" className="mt-4 rounded-xl border border-stone-200 bg-white p-4 font-sans text-sm">
    <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="m-0 font-serif text-lg font-normal">Linked tasks</h2><a className="inline-flex min-h-11 items-center text-lime-800 underline" href={`/tasks?studentId=${studentId}&dismissed=1`}>Open tasks board</a></div>
    <p className="text-xs text-slate-500">All linked tasks, including completed and dismissed tasks, prevent student deletion. Remove each link first.</p>
    {error && <p role="alert" className="text-red-700">{error}</p>}
    {!board && !error && <p role="status">Loading linked tasks…</p>}
    {board?.tasks.length === 0 && <p role="status">No linked tasks.</p>}
    <ul className="m-0 list-none divide-y divide-stone-200 p-0">{board?.tasks.map(task => <li key={task.key} className="py-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="min-w-0 break-words">{task.title}<span className="block text-xs text-slate-500">{board.columns.find(column => column.status === task.status)?.title}{task.dismissed && ' · Dismissed'}</span></span>{confirm !== task.key && <button type="button" className={button} disabled={saving || !!error} onClick={() => setConfirm(task.key)}>Remove link<span className="sr-only"> from {task.title}</span></button>}</div>{confirm === task.key && <div className="mt-2 rounded-lg bg-stone-100 p-3"><p>Remove this student relationship? The task will be kept.</p><div className="flex flex-wrap gap-2"><button type="button" className={button} disabled={saving} onClick={() => setConfirm(null)}>Keep link</button><button type="button" className={button} disabled={saving || !!error} onClick={() => void unlink(task.key)}>{saving ? 'Removing…' : 'Confirm remove link'}</button></div></div>}</li>)}</ul>
