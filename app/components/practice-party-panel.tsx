  ...
  return <dialog ref={dialog} aria-label="Practice party" className="fixed inset-y-0 right-0 left-auto m-0 h-dvh max-h-none w-full max-w-3xl overflow-y-auto border-0 bg-stone-50 p-0 text-slate-800 shadow-2xl backdrop:bg-slate-950/60" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }} onClick={event => { if (event.target !== event.currentTarget) return; const bounds = event.currentTarget.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) onClose(); }}>
    ...
    <div className="space-y-5 p-5">
      ...
      {!data ? <p>Loading practice party…</p> : <>
        ...
        {tab === 'form' && <EventSchedule key={`edit-${data.party.id}-${data.party.revision}`} data={data} saved={saved} setParentBusy={setBusy} />}
...
function Roster({ id, session, reload, refresh }: { id: number; session: PracticeSession; reload: number; refresh: () => void }) {
  const [query, setQuery] = useState(''), [page, setPage] = useState(1), [data, setData] = useState<{ students: EventStudent[]; count: number } | null>(null), [error, setError] = useState(''), [donations, setDonations] = useState<Record<number, string>>({});
  const mutation = useEventSave(`/api/practice-parties/${id}`, () => { setDonations({}); refresh(); });
  useEffect(() => { const controller = new AbortController(); setError(''); const timer = window.setTimeout(() => { requestJson<{ students: EventStudent[]; count: number }>(`/api/practice-parties/${id}/roster?${new URLSearchParams({ q: query, page: String(page) })}`, { signal: controller.signal }).then(result => { if (!controller.signal.aborted) { setData(result); setDonations({}); } }).catch(reason => { if (!controller.signal.aborted) setError(reason.message); }); }, 150); return () => { window.clearTimeout(timer); controller.abort(); }; }, [id, session.id, query, page, reload]);
  ...
  function saveDonation(student: EventStudent) {
    ...
    void mutation.save({ action: 'attendance_batch', revision: session.revision, addStudentIds: [], removeAttendanceIds: [], donations: [{ studentId: student.id, amount }] });
  }
...
function EventSchedule({ data, saved, setParentBusy }: { data: EventDetail; saved: () => void; setParentBusy: (busy: boolean) => void }) {
  ...
  return <section aria-label="Correct date and time" className="space-y-4 rounded-xl border-2 border-lime-500 bg-white p-5 shadow-sm"><h2 className="text-xl">Correct date / time</h2><p className="font-sans text-sm text-slate-500">Changes save automatically.</p><fieldset disabled={mutation.busy || mutation.uncertain} className="space-y-4 font-sans text-sm"><SessionFields value={schedule} onChange={setSchedule} /></fieldset>{mutation.error && <p role="alert" className="text-red-700">{mutation.error}</p>}</section>;
}
