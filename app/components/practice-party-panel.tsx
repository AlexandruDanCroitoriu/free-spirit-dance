...
export default function PracticePartyPanel({ id, onClose, initialTab }: { id: number; onClose: () => void; initialTab?: 'form' | 'attendance' | 'donations' }) {
  ...
  useEffect(() => { const element = dialog.current, overflow = document.body.style.overflow; document.body.style.overflow = 'hidden'; element?.showModal(); return () => { element?.close(); document.body.style.overflow = overflow; window.scrollTo(0, window.scrollY); }; }, []);
