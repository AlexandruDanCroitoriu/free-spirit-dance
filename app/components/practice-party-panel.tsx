...
export default function PracticePartyPanel({ id, onClose, initialTab }: { id: number; onClose: () => void; initialTab?: 'form' | 'attendance' | 'donations' }) {
  ...
  return <dialog ref={dialog} aria-label="Practice party" className="fixed inset-y-0 right-0 left-auto m-0 box-border h-dvh max-h-none w-dvw max-w-none overflow-y-auto border-0 bg-stone-50 p-0 text-slate-800 shadow-2xl backdrop:bg-slate-950/60 md:w-full md:max-w-3xl" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }} onClick={event => { if (event.target !== event.currentTarget) return; const bounds = event.currentTarget.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) onClose(); }}>
