...
export default function PracticePartyPanel({ id, onClose, initialTab }: { id: number; onClose: () => void; initialTab?: 'form' | 'attendance' | 'donations' }) {
  ...
  useEffect(() => { const element = dialog.current, overflow = document.body.style.overflow; document.body.style.overflow = 'hidden'; element?.showModal(); return () => { element?.close(); document.body.style.overflow = overflow; window.requestAnimationFrame(() => window.requestAnimationFrame(() => window.scrollTo(0, window.scrollY))); }; }, []);
  ...
  return <dialog ref={dialog} aria-label="Practice party" className="fixed inset-0 m-0 box-border h-dvh max-h-none w-auto max-w-none overflow-y-auto border-0 bg-stone-50 p-0 text-slate-800 shadow-2xl backdrop:bg-slate-950/60 md:inset-y-0 md:left-auto md:w-full md:max-w-3xl" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }} onClick={event => { if (event.target !== event.currentTarget) return; const bounds = event.currentTarget.getBoundingClientRect(); if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) onClose(); }}>
...
function Roster({ id, session, reload, refresh }: { id: number; session: PracticeSession; reload: number; refresh: () => void }) {
  ...
  function submit() {
    ...
    void mutation.save({ action: 'attendance_batch', revision: session.revision, addStudentIds, removeAttendanceIds, donations: donationsToSave });
  }
