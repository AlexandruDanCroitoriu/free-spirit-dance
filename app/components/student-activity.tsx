...
export default function StudentActivity({ studentId, initialPaymentId, targetPaymentId, targetPaymentKind = "payment", targetAttendanceDate, targetAttendanceCourseName, onOpenPayment }: { onOpenPayment?: (studentId: number, paymentId: number) => void; studentId: number; initialPaymentId?: number; targetPaymentId?: number; targetPaymentKind?: "payment" | "practice_attendance"; targetAttendanceDate?: string; targetAttendanceCourseName?: string | null }) {
  ...
  useEffect(() => {
    if (!data || !targetAttendanceDate || scrolledAttendanceTarget.current === targetAttendanceDate) return;
    const target = attendanceTarget.current;
    if (!target) return;
    const timeout = window.setTimeout(() => {
      target.scrollIntoView({ behavior: "smooth", block: "center" });
      scrolledAttendanceTarget.current = targetAttendanceDate;
    }, 0);
  ...
  async function deletePayment() {
    ...
    try {
      ...
      setMode(null); setNotice("Payment deleted."); setReload((value) => value + 1);
  ...
  const firstTargetAttendance = targetAttendanceDate ? data?.logs.findIndex((row) => (row.kind === "attendance" || row.kind === "missed" || row.kind === "free_missed" || row.kind === "practice_attendance" || row.kind === "free_event_attendance") && row.eventDate.slice(0, 10) === targetAttendanceDate && (targetAttendanceCourseName === undefined || row.courseName === targetAttendanceCourseName)) : -1;
  ...
  return <section aria-labelledby="student-activity-title" className="mt-6 space-y-5 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
    ...
    {data && <>
      ...
      <p className="font-sans text-sm text-slate-600">Practice sessions attended: {data.summary.eventAttendanceCount ?? 0} · Event meetings attended: {data.summary.freeEventAttendanceCount ?? 0} · Practice donations: {formatMoney(data.summary.donationsMinor ?? 0)}</p>
