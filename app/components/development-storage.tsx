...
export default function DevelopmentStorage() {
  const [selected, setSelected] = useState<"local" | "catalog" | "production" | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/development-storage", { cache: "no-store", signal: controller.signal })
      .then((response) => response.json() as Promise<{ available?: boolean; catalogAvailable?: boolean; selected: "local" | "catalog" | "production" }>)
      .then((data) => { if (data.available) { setSelected(data.selected); setCatalogAvailable(data.catalogAvailable === true); } })
      .catch(() => {});
  ...
  async function change(next: "local" | "catalog" | "production") {
  ...
  return <div className="my-4 rounded-lg border border-slate-700 p-3 text-xs">
    ...
    <div aria-label="Development database" className="flex gap-1" role="group">
      {(["local", "catalog", "production"] as const).map((value) => <button
        aria-pressed={selected === value} disabled={busy || (value === "catalog" && !catalogAvailable)} key={value}
      ...
      >{value === "local" ? "Local" : value === "catalog" ? "Catalog" : "Production"}</button>)}
    </div>
    <p className="mt-2 text-slate-400">{selected === "production" ? "Changes affect live school data." : selected === "catalog" ? "Imported catalog. Changes stay on this computer." : "Local test data and images."}</p>
    {!catalogAvailable && <p className="mt-2 text-slate-400">Catalog has not been prepared on this computer.</p>}
