...
export default function DevelopmentStorage() {
  ...
  return <div className="my-4 rounded-lg border border-slate-700 p-3 text-xs">
    ...
    <button type="button" aria-pressed={selected === "production"} disabled={busy} className={`mb-2 w-full rounded px-2 py-2 font-semibold disabled:opacity-50 ${selected === "production" ? "bg-orange-300 text-slate-950" : "bg-slate-800 text-slate-300"}`} onClick={() => void change("production")}>Production</button>
    ...
    <label className="mt-2 block text-slate-400">Local database<select aria-label="Local development database" disabled={busy} value={selected === 'production' ? '' : selected} onChange={(event) => void change(event.currentTarget.value)} className="mt-1 w-full rounded border border-slate-600 bg-slate-800 px-2 py-2 font-sans text-xs font-semibold text-slate-100 disabled:opacity-50">{selected === 'production' && <option value="" disabled hidden>Select a local database</option>}<option value="catalog">Catalog</option>{copies.map(copy => <option key={copy.id} value={copy.id}>{copy.name}</option>)}</select></label>
    <p className="mt-2 text-slate-400">{selected === "production" ? "Live production database. Changes are applied immediately." : selected === "catalog" ? "Imported catalog. Changes stay on this computer." : "Local database copy. Changes stay on this computer."}</p>
