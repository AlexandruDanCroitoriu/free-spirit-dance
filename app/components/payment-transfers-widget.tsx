...
type Payments = { payments: { id: number; purpose?: string; practiceId?: number; practiceDescription?: string; studentId: number; firstName: string; lastName: string; studentEmail: string | null; studentPicture: string | null; administratorName: string | null; administratorPicture: string | null; paidOn: string; amountMinor: number; recordedBy: string; givenToSchool: number }[]; count: number; totals: { pendingMinor: number; givenMinor: number } };
...
export default function PaymentTransfersWidget() {
  ...
  return <section aria-busy={loading} aria-labelledby="payment-transfers-title" className="min-w-0 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm lg:col-span-full">
    ...
    {data && <><div className="mb-4 flex flex-wrap gap-4 font-sans text-sm"><p className="m-0 rounded-lg bg-amber-50 p-3 text-amber-900">Unconfirmed handovers: <strong>{formatMoney(data.totals.pendingMinor)}</strong></p><p className="m-0 rounded-lg bg-lime-50 p-3 text-lime-900">Given to school: <strong>{formatMoney(data.totals.givenMinor)}</strong></p></div>
      <div className="overflow-x-auto"><table className="w-full text-left font-sans text-sm"><caption className="sr-only">Payments from all students, newest first</caption><thead className="bg-stone-50 text-xs text-slate-500"><tr>{["Date", "Student", "Amount", "Collected by", "School transfer"].map((label) => <th key={label} scope="col" className="p-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-stone-200">{data.payments.map((payment) => <tr key={`${payment.purpose}-${payment.id}`}><td className="whitespace-nowrap p-3">{formatLogDate(payment.paidOn)}</td><td className="p-3"><a className="flex items-center gap-2 underline" href={`/students/${payment.studentId}`}><PaymentAvatar picture={payment.studentPicture} name={payment.firstName.trim() || payment.studentEmail || "Student"} /><span>{payment.firstName.trim() ? `${payment.firstName} ${payment.lastName}`.trim() : payment.studentEmail || "Student"}</span></a></td><td className="whitespace-nowrap p-3">{formatMoney(payment.amountMinor)}{payment.purpose === "practice_donation" && <><p className="text-xs">Donation · {payment.practiceDescription}</p></>}</td><td className="p-3 text-xs"><span className="flex items-center gap-2" title={payment.recordedBy}><PaymentAvatar picture={payment.administratorPicture} name={payment.administratorName?.trim() || payment.recordedBy} /><span className="break-words">{payment.administratorName?.trim() || payment.recordedBy}</span></span></td><td className="p-3"><>{payment.purpose === "practice_donation" ? <a className="underline" href={`/practice-parties/${payment.practiceId}`}>View practice finances</a> : <PaymentTransferCheckbox paymentId={payment.id} studentId={payment.studentId} checked={payment.givenToSchool === 1} disabled={loading || !!error} />}</></td></tr>)}{!data.payments.length && <tr><td colSpan={5} className="p-6 text-center text-slate-500">No payments match this filter.</td></tr>}</tbody></table></div>
