...
export default function StudentCard({ presentationOnly = false, student, onOpen, courses }: StudentCardProps) {
  ...
  return <article className={`relative flex min-h-20 flex-wrap items-center gap-x-4 transition-colors ${presentationOnly ? "justify-center gap-y-3" : "sm:flex-nowrap pr-5 hover:bg-stone-50"}`}>
    ...
    <div className={presentationOnly ? "flex w-full justify-center" : "h-20 w-20 shrink-0"}>{avatar}</div>
    <div className={presentationOnly ? "w-full min-w-0 text-center" : "min-w-0 flex-1"}><h2 className={presentationOnly ? "m-0 break-words text-2xl font-normal" : "m-0 truncate text-lg font-normal"}>{fullName}</h2></div>
    {courses !== undefined && <div aria-label="Assigned courses" className="order-last w-full shrink-0 pb-3 pl-24 font-sans sm:order-none sm:w-44 sm:py-3 sm:pl-0 lg:w-56">
