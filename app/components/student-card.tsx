...
export default function StudentCard({ presentationOnly = false, student, onOpen, onOpenPhoto, courses }: StudentCardProps) {
  ...
  const displayedName = student.nickname ? `${fullName} (${student.nickname})` : fullName;
  ...
  const avatar = <span className={`flex items-center justify-center overflow-hidden bg-lime-200 font-sans font-bold text-slate-800 ${presentationOnly ? "h-24 w-24 rounded-full text-2xl" : "h-full w-full"}`}>{student.picture ? <img src={student.picture} alt="" className="h-full w-full object-cover" /> : `${student.firstName[0]}${student.lastName[0]}`}</span>;
  ...
  return <article className={`relative min-h-20 items-center transition-colors ${presentationOnly ? "flex flex-wrap justify-center gap-x-4 gap-y-3" : "grid grid-cols-[3rem_minmax(0,1fr)_auto_auto] gap-x-2 gap-y-1 p-3 sm:flex sm:flex-nowrap sm:gap-x-4 sm:p-0 sm:pr-5 hover:bg-stone-50"}`}>
    ...
    <div className={presentationOnly ? "w-full min-w-0 text-center" : "col-start-2 row-start-1 min-w-0 sm:flex-1"}><h2 className={presentationOnly ? "m-0 break-words text-2xl font-normal" : "m-0 break-words text-base font-normal sm:truncate sm:text-lg"}>{displayedName}</h2></div>
    ...
    {!presentationOnly && <span className={`hidden font-sans text-xs font-bold uppercase tracking-wider sm:inline ${student.active ? "text-lime-700" : "text-slate-400"}`}>{student.active ? "Active" : "Inactive"}</span>}
    {!presentationOnly && student.phone && <a href={`tel:${student.phone}`} aria-label={`Call ${fullName} at ${student.phone}`} className="relative z-20 col-start-3 row-start-1 row-span-2 flex min-h-10 min-w-10 shrink-0 items-center justify-center rounded-md border border-stone-300 bg-white p-2 font-sans text-xs font-semibold text-slate-600 transition-colors hover:bg-stone-100 focus:outline-none focus:ring-2 focus:ring-lime-600 lg:gap-2 lg:px-3">
    ...
    {!presentationOnly && student.phone && <a href={`https://wa.me/${whatsappNumber}`} aria-label={`Start a WhatsApp conversation with ${fullName}`} className="relative z-20 col-start-4 row-start-1 row-span-2 flex min-h-10 min-w-10 shrink-0 items-center justify-center rounded-md border border-emerald-300 bg-white p-2 font-sans text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 lg:gap-2 lg:px-3" rel="noreferrer" target="_blank">
