...
export default function OperationNotification({ message, kind = "success", onDismiss }: {
  ...
  return <div ref={element} popover="manual" role={kind === "error" ? "alert" : "status"}
    className={`fixed inset-auto top-4 right-4 m-0 w-[calc(100%-2rem)] max-w-sm rounded-xl border p-4 shadow-xl font-sans text-sm ${kind === "error" ? "border-red-200 bg-red-50 text-red-800" : "border-lime-200 bg-lime-50 text-lime-900"}`}>
