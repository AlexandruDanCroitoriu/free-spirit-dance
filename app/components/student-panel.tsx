...
export default function StudentPanel({ id, onClose, onUpdate, onDelete, editPaymentId, targetPaymentId, targetPaymentKind = "payment", attendanceDate }: { id: number; onClose: () => void; onUpdate: (student: Student) => void; onDelete: (id: number) => void; editPaymentId?: number; targetPaymentId?: number; targetPaymentKind?: "payment" | "practice_attendance"; attendanceDate?: string }) {
  ...
  async function saveField(field: Exclude<Field, "id" | "picture">, value: string) {
    ...
    if ((field === "facebookUrl" || field === "instagramUrl") && value && !/^https:\/\/[^\s]+$/i.test(value)) { setError("Social profile links must be HTTPS URLs or left empty."); return; }
    pendingSaves.current += 1;
  ...
  return <dialog ref={dialog} aria-label={student ? `${student.firstName} ${student.lastName}` : "Student details"} aria-modal="true" className="fixed inset-0 m-0 box-border h-dvh max-h-none w-auto max-w-none overflow-y-auto border-0 bg-stone-50 p-0 text-slate-800 shadow-2xl backdrop:bg-slate-950/60 md:inset-y-0 md:left-auto md:w-full md:max-w-3xl" onCancel={(event) => { event.preventDefault(); closePanel(); }} onClick={(event) => {
    ...
    <section aria-label="Student info" className="mt-4 overflow-hidden rounded-xl border border-stone-200 bg-white px-5 shadow-sm">
      ...
      {editableFields.map(({ key, label, type }) => <div key={key} className="border-b border-stone-200 py-4"><label className="block font-sans"><span className="flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-wider text-slate-400"><span>{label}</span>{savingField === key && <span className="normal-case tracking-normal text-lime-700">Saving...</span>}</span><input disabled={savingPhoto} type={type ?? "text"} placeholder={key === "facebookUrl" ? "https://facebook.com/…" : key === "instagramUrl" ? "https://instagram.com/…" : undefined} {...(key === "birthDate" ? { max: new Date().toISOString().slice(0, 10) } : {})} {...(key === "phone" ? { inputMode: "numeric" as const, minLength: 10, pattern: "[0-9]{10,}", title: "Enter at least 10 numbers." } : {})} value={drafts[key]} onChange={(event) => setDrafts((current) => ({ ...current, [key]: event.target.value }))} onBlur={() => void saveField(key, drafts[key])} className="mt-2 w-full rounded-md border border-stone-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition-colors focus:border-lime-600 focus:ring-1 focus:ring-lime-600" /></label></div>)}
