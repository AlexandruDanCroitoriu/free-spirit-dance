...
export async function GET() {
  ...
  try {
    ...
    const data = await db.batch([
      ...
      db.prepare(`SELECT p.student_id AS studentId, a.course_id AS courseId, p.paid_on AS paidOn, a.allowance, p.notes, ${coverageThrough} AS coverageThrough FROM payment_course_allowances a JOIN student_payments p ON p.id = a.payment_id ORDER BY p.paid_on, p.id`),
    ...
    for (const key of new Set([...assigned, ...payments.keys(), ...attendance.keys(), ...absences.keys()])) {
      ...
      const balance = courseCreditBalance(course, schedules.get(String(courseId)) ?? [], classes.get(String(courseId)) ?? [], payments.get(key) ?? [], attendance.get(key) ?? [], now, undefined, undefined, undefined, historicalAbsences === undefined ? undefined : (absences.get(key) ?? []).map((row) => row.startsAt), (startsAt) => unpaidAttendances.push(startsAt));
