...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows = await db.prepare(`SELECT s.id, s.first_name AS firstName, s.last_name AS lastName, s.picture, s.active,
      ...
      FROM students s ORDER BY s.last_name COLLATE NOCASE, s.first_name COLLATE NOCASE, s.id`).bind(slot.courseId, slot.courseId, `${slot.classDate}T${slot.startTime}:00`).all<ClassStudent>();
...
export async function POST(request: Request) {
  ...
  if (!slot || !validIds(additions) || !validIds(removals) || additions.length + removals.length < 1 || additions.length + removals.length > 500 || new Set([...additions, ...removals]).size !== additions.length + removals.length) return json({ error: "Select 1–500 attendance changes for a valid class." }, 400);
  ...
  try {
    ...
    const results = await db.batch([
      ...
      ...additions.map((studentId: number) => db.prepare(`INSERT INTO attendance (class_id, student_id, course_id, course_name, attended_at, recorded_by, recorded_at, notes, request_key, request_payload)
        ...
        .bind(slot.courseId, slot.classDate, slot.startTime, studentId, slot.courseId, scheduled.courseName, attendedAt, email, now, `calendar-${slot.courseId}-${slot.classDate}-${slot.startTime}-${studentId}`, JSON.stringify({ ...slot, studentId }), studentId, slot.courseId, attendedAt)),
    ...
    return json({ recorded: results.slice(2, 2 + additions.length).reduce((sum, result) => sum + result.meta.changes, 0), removed: results.slice(2 + additions.length).reduce((sum, result) => sum + result.meta.changes, 0), studentIds: additions, removeStudentIds: removals });
  } catch (error) {
