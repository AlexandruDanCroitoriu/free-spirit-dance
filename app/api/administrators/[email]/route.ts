...
export async function PATCH(request: Request, context: { params: Promise<{ email: string }> }) {
  ...
  const input = await request.json().catch(() => null) as { dashboard?: unknown; students?: unknown; courses?: unknown; practiceParties?: unknown; qrCodes?: unknown; tasks?: unknown } | null;
  if (!input || typeof input.dashboard !== "boolean" || typeof input.students !== "boolean" || typeof input.courses !== "boolean" || typeof input.practiceParties !== "boolean" || typeof input.qrCodes !== "boolean" || (input.tasks !== undefined && typeof input.tasks !== "boolean")) return Response.json({ error: "All permissions are required." }, { status: 400 });
  try {
    const result = await env.DB.prepare("UPDATE administrator_permissions SET can_dashboard = ?, can_students = ?, can_courses = ?, can_practice_parties = ?, can_qr_codes = ?, can_tasks = COALESCE(?, can_tasks) WHERE email = ? RETURNING email, can_dashboard, can_students, can_courses, can_practice_parties, can_qr_codes, can_tasks").bind(input.dashboard ? 1 : 0, input.students ? 1 : 0, input.courses ? 1 : 0, input.practiceParties ? 1 : 0, input.qrCodes ? 1 : 0, input.tasks === undefined ? null : input.tasks ? 1 : 0, email).first<PermissionRow>();
