...
export async function GET() {
  try {
    const result = await env.DB.prepare("SELECT p.email, p.can_dashboard, p.can_students, p.can_courses, p.can_practice_parties, p.can_qr_codes, COALESCE(a.name, '') AS name, a.picture FROM administrator_permissions p LEFT JOIN admin_profiles a ON a.email = p.email ORDER BY p.email COLLATE NOCASE").all<AdministratorRow>();
