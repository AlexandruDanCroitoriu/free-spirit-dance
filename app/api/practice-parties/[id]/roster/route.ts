...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) { return eventHandler(async () => {
  ...
  const rows = await env.DB.batch([
    env.DB.prepare(`SELECT s.id, s.first_name AS firstName, s.last_name AS lastName, s.email, s.picture, s.active, a.id AS attendanceId, a.donation_amount_minor AS donationAmountMinor FROM students s LEFT JOIN practice_attendance a ON a.student_id = s.id AND a.practice_id = ? ${where} ORDER BY s.active DESC, s.first_name COLLATE NOCASE, s.last_name COLLATE NOCASE, s.id LIMIT 100 OFFSET ?`).bind(id, search, search, (page - 1) * 100),
