...
export type StudentActivity = {
  ...
  logs: { id: number; kind: "attendance" | "payment" | "missed" | "cancelled"; complimentary?: number; complimentaryBy?: string | null; complimentaryAt?: string | null; courseId?: number | null; eventDate: string; courseName: string | null; amountMinor: number | null; notes: string; recordedBy: string; recordedAt: string | null; allocations: { courseId: number; courseName: string; allowance: number; coverage?: PaymentCoverage }[] }[];
  logsPage: number;
  logsPageSize: number;
  logsCount: number;
