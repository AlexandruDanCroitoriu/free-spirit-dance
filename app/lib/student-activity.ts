...
export type ActivitySummary = { eventAttendanceCount?: number; freeEventAttendanceCount?: number; donationsMinor?: number; missedClasses: number; attendanceCount: number; paymentCount: number; paidAllowance: number; remainingAllowance: number; excessAttendance: number; totalPaidMinor: number };
export type StudentActivity = {
  canRecordFuturePayments: boolean;
  logs: (GroupPaymentDetails & { id: number; kind: "attendance" | "payment" | "missed" | "free_missed" | "cancelled" | "practice_attendance" | "free_event_attendance"; practiceId?: number; eventId?: number; voidedAt?: string | null; givenToSchool?: number; receivedMethod?: string; complimentary?: number; complimentaryBy?: string | null; complimentaryAt?: string | null; courseId?: number | null; eventDate: string; courseName: string | null; amountMinor: number | null; notes: string; recordedBy: string; recordedAt: string | null; allocations: { courseId: number; courseName: string; allowance: number; coverage?: PaymentCoverage }[] })[];
  logsPage: number;
  logsPageSize: number;
  logsCount: number;
