...
export type ClassStudent = { id: number; firstName: string; lastName: string; picture: string | null; active: number; assigned: number; attended: number; complimentary?: number; freeMissed?: number };
export type ClassRoster = { cancelled: boolean; canManageClass: boolean; canEdit: boolean; canRemoveClass: boolean; courseName: string; createdBy: string | null; creatorName: string | null; endTime: string | null; rentCostMinor: number; rentPaid: boolean; courses: { id: number; name: string }[]; students: ClassStudent[] };
