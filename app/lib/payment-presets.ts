...
export type PaymentPreset = { id: number; name: string; amountMinor: number; studentCount: number; courseId: number | null; allocations: { courseId: number; courseName: string; allowance: number }[] };
export type PresetInput = { name: string; amountMinor: number; studentCount: number; allocations: { courseId: number; allowance: number }[] };
export function parsePreset(value: unknown): PresetInput | string {
  ...
  if (typeof studentCount !== "number" || !Number.isSafeInteger(studentCount) || studentCount < 1 || studentCount > 10000) return "Enter a whole number of students between 1 and 10,000 (at least 2 for a group).";
  ...
  return { name: input.name.trim(), amountMinor, studentCount, allocations };
}
export const presetQuery = `SELECT p.id, p.name, p.amount_minor AS amountMinor, p.student_count AS studentCount, p.course_id AS ownedCourseId, a.course_id AS courseId, c.name AS courseName, a.allowance
  FROM payment_presets p LEFT JOIN payment_preset_courses a ON a.preset_id = p.id LEFT JOIN courses c ON c.id = a.course_id`;
type PresetRow = { id: number; name: string; amountMinor: number; studentCount: number; ownedCourseId: number | null; courseId: number | null; courseName: string | null; allowance: number | null };
export function serializePresets(rows: PresetRow[]): PaymentPreset[] {
  const items = new Map<number, PaymentPreset>();
  for (const row of rows) {
    if (!items.has(row.id)) items.set(row.id, { id: row.id, name: row.name, amountMinor: row.amountMinor, studentCount: row.studentCount, courseId: row.ownedCourseId, allocations: [] });
