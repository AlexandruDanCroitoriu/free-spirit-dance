...
type Course = { id: number; name: string; recurrenceOne: Recurrence; dayOne: string; startTimeOne: string; endTimeOne: string; recurrenceTwo: Recurrence | null; dayTwo: string | null; startTimeTwo: string | null; endTimeTwo: string | null };
type CourseForm = Omit<Course, "id">;
type ApiError = { error?: string };
const days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
const emptyForm: CourseForm = { name: "", recurrenceOne: "weekly", dayOne: "Monday", startTimeOne: "18:00", endTimeOne: "19:00", recurrenceTwo: null, dayTwo: null, startTimeTwo: null, endTimeTwo: null };
...
function displaySchedule(recurrence: Recurrence, day: string, startTime: string, endTime: string) {
  const frequencies: Record<Recurrence, string> = { weekly: "Every week", twice_weekly: "Two times a week" };
  return { day, time: `${displayTime(startTime)}–${displayTime(endTime)}`, frequency: frequencies[recurrence] };
}
...
export default function CoursesPage() {
  ...
  function editCourse(course: Course) {
    setEditingId(course.id); setForm({ name: course.name, recurrenceOne: course.recurrenceOne, dayOne: course.dayOne, startTimeOne: course.startTimeOne, endTimeOne: course.endTimeOne, recurrenceTwo: course.recurrenceTwo, dayTwo: course.dayTwo, startTimeTwo: course.startTimeTwo, endTimeTwo: course.endTimeTwo });
    setError(""); setFormOpen(true);
  }
  function setFrequency(recurrenceOne: Recurrence) {
    setForm((current) => ({ ...current, recurrenceOne, recurrenceTwo: recurrenceOne === "twice_weekly" ? "twice_weekly" : null, dayTwo: recurrenceOne === "twice_weekly" ? (current.dayTwo ?? "Thursday") : null, startTimeTwo: recurrenceOne === "twice_weekly" ? (current.startTimeTwo ?? "18:00") : null, endTimeTwo: recurrenceOne === "twice_weekly" ? (current.endTimeTwo ?? "19:00") : null }));
  }
  ...
  return <main className="flex-1 bg-stone-50 px-6 py-6 text-slate-800 md:px-12"><div className="mx-auto max-w-5xl">
    {formOpen && <div className="fixed inset-0 z-40 overflow-y-auto bg-slate-950/70 p-4 md:left-64" role="presentation"><div aria-labelledby="course-dialog-title" aria-modal="true" className="mx-auto mt-8 w-full max-w-xl rounded-xl border border-stone-200 bg-white p-6 shadow-2xl" role="dialog"><form className="space-y-5" onSubmit={saveCourse}>
      ...
      <fieldset className="rounded-lg border border-stone-200 p-4"><legend className="px-1 font-sans text-xs font-bold uppercase tracking-wider text-slate-500">Class</legend><SlotFields recurrence={form.recurrenceOne} day={form.dayOne} startTime={form.startTimeOne} endTime={form.endTimeOne} secondDay={hasSecondSlot ? (form.dayTwo ?? "Thursday") : null} secondStartTime={hasSecondSlot ? (form.startTimeTwo ?? "18:00") : null} secondEndTime={hasSecondSlot ? (form.endTimeTwo ?? "19:00") : null} onChange={(values) => { setFrequency(values.recurrence); setForm((current) => ({ ...current, dayOne: values.day, startTimeOne: values.startTime, endTimeOne: values.endTime })); }} onSecondChange={(values) => setForm((current) => ({ ...current, dayTwo: values.day, startTimeTwo: values.startTime, endTimeTwo: values.endTime }))} /></fieldset>
...
function SlotFields({ recurrence, day, startTime, endTime, secondDay, secondStartTime, secondEndTime, onChange, onSecondChange }: { recurrence: Recurrence; day: string; startTime: string; endTime: string; secondDay: string | null; secondStartTime: string | null; secondEndTime: string | null; onChange: (value: { recurrence: Recurrence; day: string; startTime: string; endTime: string }) => void; onSecondChange: (value: { day: string; startTime: string; endTime: string }) => void }) {
  ...
  return <div className="grid items-start gap-4 sm:grid-cols-4"><label className="font-sans text-xs font-semibold text-slate-600">Repeats<select className={inputClass} value={recurrence} onChange={(event) => onChange({ recurrence: event.target.value as Recurrence, day, startTime, endTime })}><option value="weekly">Every week</option><option value="twice_weekly">Two times a week</option></select></label><div className="space-y-3"><label className="block font-sans text-xs font-semibold text-slate-600">Day<select className={inputClass} value={day} onChange={(event) => onChange({ recurrence, day: event.target.value, startTime, endTime })}>{days.map((weekday) => <option key={weekday}>{weekday}</option>)}</select></label>{secondDay !== null && secondStartTime !== null && secondEndTime !== null && <label className="block"><span className="sr-only">Second day</span><select aria-label="Second day" className={inputClass} value={secondDay} onChange={(event) => onSecondChange({ day: event.target.value, startTime: secondStartTime, endTime: secondEndTime })}>{days.map((weekday) => <option key={weekday}>{weekday}</option>)}</select></label>}</div><div className="space-y-3"><label className="block font-sans text-xs font-semibold text-slate-600">Start time<input className={inputClass} required type="time" value={startTime} onChange={(event) => onChange({ recurrence, day, startTime: event.target.value, endTime })} /></label>{secondDay !== null && secondStartTime !== null && secondEndTime !== null && <label className="block"><span className="sr-only">Second class start time</span><input aria-label="Second class start time" className={inputClass} required type="time" value={secondStartTime} onChange={(event) => onSecondChange({ day: secondDay, startTime: event.target.value, endTime: secondEndTime })} /></label>}</div><div className="space-y-3"><label className="block font-sans text-xs font-semibold text-slate-600">End time<input className={inputClass} required type="time" value={endTime} onChange={(event) => onChange({ recurrence, day, startTime, endTime: event.target.value })} /></label>{secondDay !== null && secondStartTime !== null && secondEndTime !== null && <label className="block"><span className="sr-only">Second class end time</span><input aria-label="Second class end time" className={inputClass} required type="time" value={secondEndTime} onChange={(event) => onSecondChange({ day: secondDay, startTime: secondStartTime, endTime: event.target.value })} /></label>}</div></div>;
}
