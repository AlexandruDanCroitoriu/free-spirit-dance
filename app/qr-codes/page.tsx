...
export default function QrCodesPage() {
  ...
  const [open, setOpen] = useState(false); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState(""); const [message, setMessage] = useState("");
  ...
  function create() { clearPreview(); setEditingId(null); setForm(emptyForm); setPendingImage(null); setError(""); setMessage(""); setOpen(true); }
  ...
  function edit(code: QrCode) { clearPreview(); setEditingId(code.id); setForm({ name: code.name, destinationUrl: code.destinationUrl, active: code.active, imageMode: code.imageMode }); setPendingImage(null); setError(""); setMessage(""); setOpen(true); }
  ...
  async function save(event: React.FormEvent) {
    ...
    setCodes((current) => editingId ? current.map((code) => code.id === data.id ? data : code) : [data, ...current]); close(); setMessage(editingId ? "QR code updated. The printed redirect address is unchanged." : "QR code created."); setSaving(false);
  }
  async function toggle(code: QrCode) {
    const nextActive = !code.active;
    setUpdatingStatusId(code.id); setError(""); setMessage("");
    try {
      ...
      setMessage(nextActive ? "QR code activated." : "QR code deactivated.");
  ...
  async function copyUrl(url: string, label: string) { try { await navigator.clipboard.writeText(url); setError(""); setMessage(`${label} copied.`); } catch { setMessage(""); setError("Could not copy the URL."); } }
  ...
  return <main className="flex-1 bg-stone-50 px-6 py-6 text-slate-800 md:px-12"><div className="mx-auto max-w-5xl">
    ...
    {(error || message) && <div className={`mb-4 rounded-lg border px-4 py-3 text-sm ${error ? "border-red-200 bg-red-50 text-red-700" : "border-lime-200 bg-lime-50 text-lime-800"}`}>{error || message}</div>}
