...
export default function HomePage() {
  ...
  return <main className="flex-1 px-4 py-6 text-slate-800 md:px-12">
    {ready && <div className={`grid w-full grid-cols-1 items-start gap-4 lg:gap-5 xl:grid-cols-[minmax(0,2fr)_minmax(20rem,1fr)] ${widgets.calendar && widgets.balances ? "xl:items-stretch" : ""}`}>
      <Suspense fallback={null}>{widgets.calendar && <div className="min-w-0 xl:[&>section]:!mx-0 xl:[&>section]:!max-w-none"><CourseCalendarWidget /></div>}</Suspense>
      <Suspense fallback={null}>{widgets.balances && <div className={widgets.calendar ? "xl:[&>section]:h-full" : ""}><StudentBalancesWidget /></div>}</Suspense>
