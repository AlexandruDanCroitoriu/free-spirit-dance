...
export default function PaymentsPage() {
  ...
  return <main className="flex-1 bg-stone-50 px-6 py-6 text-slate-800 md:px-12"><div className="mx-auto max-w-5xl">
    ...
    {open && data && <dialog ref={dialog} aria-labelledby="preset-title" aria-modal="true" className="fixed inset-y-0 right-0 left-auto m-0 h-dvh max-h-none w-full max-w-xl overflow-y-auto border-0 bg-white p-6 text-slate-800 shadow-2xl backdrop:bg-slate-950/60" onCancel={(event) => { event.preventDefault(); close(); }} onClick={(event) => {
      if (event.target !== event.currentTarget) return;
      const bounds = event.currentTarget.getBoundingClientRect();
      if (event.clientX < bounds.left || event.clientX > bounds.right || event.clientY < bounds.top || event.clientY > bounds.bottom) close();
    }}>
      <div className="flex items-center justify-between gap-4"><h2 id="preset-title" className="m-0 text-xl font-normal">{editingId === null ? "Add payment preset" : "Edit payment preset"}</h2><button type="button" aria-label="Close payment preset panel" className={button} disabled={busy} onClick={close}>×</button></div><p className="font-sans text-sm text-slate-500">Changes apply to future payments. Recorded payments keep their original amount and allowances.</p>
