...
Calendar attendance requires both Dashboard and Students permissions. Student activity, course assignments, and balances use Students permission. Payment presets are managed from the Courses page; Students permission also permits reading presets when recording a student payment. The calendar schedule requires Dashboard permission. Practice Parties requires its own permission; adding and editing parties requires Practice Parties, attendance additionally requires Students, and donations additionally require Payments. Apply migration `0037_practice_parties.sql` before deploying this feature.
